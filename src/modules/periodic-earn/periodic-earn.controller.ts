...
export class PeriodicEarnController {
    ...
    async register(
      ...
      return await this.periodicEarnService.add(dto, req.user.user_id, req.user.role)
    }  
    ...
    async listEarns(
        ...
        return await this.periodicEarnService.listTransactions(group_id, req.user.user_id, req.user.role);
    }
    ...
    async getPeriodicEarnById(
        ...
        return await this.periodicEarnService.getById(periodicEarn_id, req.user.user_id, req.user.role);
    }
    ...
    @ApiParam({name: 'periodicEarnId', description: 'periodicEarn id'})
    @ApiParam({name: 'status', description: 'periodicEarn status', type: 'enum', enum: EStatus})
    @ApiResponse({ status: 500, description: 'Internal Server Error' })
    @ApiResponse({ status: 400, description: 'Not Found Error' })
    async editStatus(
        @Req() req: any,
