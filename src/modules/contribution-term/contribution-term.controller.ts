...
export class ContributionTermController {
  constructor(private readonly termsService: ContributionTermService) {}
  ...
  async register(
    ...
    return await this.termsService.openContributionTerm(
      createDto,
      req.user.role,
      req.user.user_id,
    );
  ...
  async getContributionTerms(
    ...
    return await this.termsService.getContributionTerms(
      group_id,
      role,
      user_id,
    );
  ...
  async getActiveContributionTerm(
    ...
    const role = req.user.role;
    const user_id = req.user.user_id;
    return await this.termsService.getActiveContributionTerm(
      group_id,
      role,
      user_id,
    );
  ...
  @ApiResponse({ status: 500, description: 'Internal Server Error' })
  async inactivate(@Req() req: any, @Param('term') term: string): Promise<any> {
    return await this.termsService.inactiveContributionTerm(
      term,
      req.user.role,
      req.user.user_id,
    );
