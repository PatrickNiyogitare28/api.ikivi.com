...
export class ContributionTermService {
    ...
    public async openContributionTerm(createTermDto: CreateContributionTermDto, role: EUserRole, user_id: string){
        ...
        const isGroupMember = await this.groupMembersService.findGroupMemberExists(user_id, group);

        if(role !== EUserRole.SYSTEM_ADMIN && groupExists.group_owner.id != user_id && !isGroupMember) throw new BadRequestException("Access denied to perform this action");
    ...
    public async getContributionTerms(group_id: string, role: EUserRole, user_id: string){
        ...
        if(role !== EUserRole.SYSTEM_ADMIN && groupExists.group_owner.id != user_id && !isGroupMember) throw new BadRequestException("Access denied to perform this action");
        const terms = await this.contributionTermRepository.find({ where: { group: group_id } });
        return terms;
    }

    public async getActiveContributionTerm(group_id: string, role: EUserRole, user_id: string){
        const groupExists = await this.groupService.findGroupById(group_id);
        if(!groupExists) throw new NotFoundException("Group not found");
        const isGroupMember = await this.groupMembersService.findGroupMemberExists(user_id, group_id);
        if(role !== EUserRole.SYSTEM_ADMIN && groupExists.group_owner.id != user_id && !isGroupMember) throw new BadRequestException("Access denied to perform this action");
        const term = await this.contributionTermRepository.findOne({ where: { group: group_id, status: EStatus.ACTIVE } });
        if(!term) return {success: true, message: 'No active group term found'}
        return term;
    }

    public async inactiveContributionTerm(term_id: string, role: EUserRole, user_id: string){
        const term = await this.contributionTermRepository.findOne({where: {id: term_id}});
        ...
        if(!groupExists) throw new NotFoundException("Group not found");
        if(role !== EUserRole.SYSTEM_ADMIN && groupExists.group_owner.id != user_id) throw new BadRequestException("Access denied to perform this action");
    ...
    public async _getContributionTermById(id: string){
      try{
        const contribution = await this.contributionTermRepository.findOne({where: {id, status: EStatus.ACTIVE}});
