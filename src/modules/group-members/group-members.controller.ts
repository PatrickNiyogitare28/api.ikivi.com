...
export class GroupMembersController {
    ...
    @ApiResponse({ status: 200, description: 'Group found', type: GroupMembersEntity })
    @ApiResponse({ status: 404, description: 'Group not found' })
    async getGroupById(
        @Param('group') group_id: string,
        @Req() req: any,        
        ): Promise<any> {
      return await this.groupMembersService.getGroupMembers(group_id, req.user.role, req.user.user_id)
    }
