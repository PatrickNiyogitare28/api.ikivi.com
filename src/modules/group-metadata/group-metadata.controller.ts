...
export class GroupMetadataController {
  ...
  @ApiResponse({
    status: 201,
    description: 'Group  metadata added successfully',
    type: GroupMetadataEntity,
  })
  ...
  async create(
    ...
    return this.groupMetadataService.addGroupMetadata(
      createGroupMetadataDto,
      id,
      req.user.role,
      req.user.user_id,
  ...
  @ApiResponse({
    status: 404,
    description: 'Group or Group metadata not found',
  })
  @ApiResponse({ status: 500, description: 'Internal Server Error' })
  @ApiParam({ name: 'id', description: 'Group id' })
  @Get('/group/:id')
  async getGroupMetadata(@Req() req: any, @Param('id') id: string) {
    return this.groupMetadataService.getGroupMetadata(
      id,
      req.user.role,
      req.user.user_id,
    );
  ...
  @ApiParam({ name: 'id', description: 'Group id' })
  @Get('/:id')
  async getMetadataById(@Req() req: any, @Param('id') id: string) {
    return this.groupMetadataService.getGroupMetadata(
      id,
      req.user.role,
      req.user.user_id,
  ...
  async updatedMetadata(
    ...
    return this.groupMetadataService.updateGroupMetadata(
      id,
      updateMetadataDto,
      req.user.role,
      req.user.user_id,
  ...
  @Delete('/:id')
  @HttpCode(200)
  async deleteMetadata(@Req() req: any, @Param('id') id: string) {
    return this.groupMetadataService.deleteGroupMetadata(
      id,
      req.user.role,
      req.user.user_id,
    );
