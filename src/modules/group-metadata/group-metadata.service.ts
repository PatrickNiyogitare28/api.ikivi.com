...
export class GroupMetadataService {
  ...
  public async addGroupMetadata(
    groupMetadata: CreateGroupMetadataDto,
    group_id: string,
    role: EUserRole,
    user_id: string,
  ) {
    ...
    try {
      const newMetadata = await this.groupMetadataRepository.save({
        ...groupMetadata,
        group: group.id,
      });
      return {
  ...
  public async getGroupMetadataList() {
    try {
      ...
      if (!metadataList || metadataList.length === 0)
        throw new NotFoundException('No group metadata found');
      return {
        success: true,
        data: metadataList,
      };
  ...
  public async getGroupMetadataById(
    metadata_id: string,
    role: EUserRole,
    user_id: string,
  ) {
    const metadata = await this.groupMetadataRepository.findOne({
  ...
  public async updateGroupMetadata(
    metadata_id: string,
    updateGroupMetadataDto: UpdateGroupMetadataDto,
    role: EUserRole,
    user_id: string,
  ) {
    const metadata = await this.groupMetadataRepository.findOne({
    ...
    try {
      const updatedMetadata = await this.groupMetadataRepository.save({
        ...metadata,
        ...updateGroupMetadataDto,
      });
      return {
  ...
  public async deleteGroupMetadata(
    metadata_id: string,
    role: EUserRole,
    user_id: string,
  ) {
  ...
  public async _findMetadataByGroupId(id: string) {
    try {
      return await this.groupMetadataRepository.findOne({
        where: { group: id },
      });
    } catch (e) {
      throw new InternalServerErrorException(e.message);
