...
export class JoinRequestsController {
    ...
    async getByGroup(
        ...
        return this.joinRequestsService.getRequestsByGroup(group_id, req.user.role, req.user.user_id);
    }

    @ApiResponse({status: 200, description: 'Successfully fetched requests'})
    @ApiParam({name: 'group', description: 'Group id'})
    @ApiParam({name: 'status', description: 'Status', type: 'enum', enum: ERequestStatus})
    ...
    async getByGroupAndStatus(
        ...
        return this.joinRequestsService.getRequestsByGroupAndStatus(group_id, status,req.user.role, req.user.user_id);
    }
    ...
    async cancelRequest(
        ...
        return this.joinRequestsService.cancelRequest(id, req.user.role, req.user.user_id);
    }
}
