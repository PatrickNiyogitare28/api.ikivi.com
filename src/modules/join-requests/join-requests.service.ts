...
export class JoinRequestsService {
    ...
    public async requestGroupJoin(newJoinRequest: NewJoinRequestDto, user_id: string){
        const codeExists = await this.joinCodesService.findActiveCode(newJoinRequest.code);
            if(!codeExists) throw new BadRequestException("Code not found or not activated");
            const requestExists = await this._findRequestByUserId(user_id, codeExists.id);
            if(requestExists?.status == ERequestStatus.APPROVED) throw new BadRequestException("You are already a member");
            if(requestExists?.status == ERequestStatus.PENDING) throw new BadRequestException("You already requested to join this group");
            if(requestExists?.status == ERequestStatus.REJECTED) throw new BadRequestException("You request was rejected");
            try{
            const request  = await this.joinRequestRepository.save({join_code: codeExists.id, user: user_id, group: codeExists.group })
            return {
                success: true,
